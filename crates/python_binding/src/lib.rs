...
mod globlin {
    ...
    mod test {
        ...
        fn test_glob_builder_methods() {
            ...
            let glob = glob.bracket_expansion();
            ...
            assert_eq!(glob.flags, ::globlin::flags::ESCAPE);
            let glob = glob.no_path_separator();
            assert_eq!(glob.flags, 0);
        }

        #[test]
        fn test_glob_from_flags() {
            let glob = Glob::from_flags(::globlin::flags::GLOB_STAR as i32).unwrap();
            assert_eq!(glob.flags, ::globlin::flags::GLOB_STAR);
        }
